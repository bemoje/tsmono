...
export function getApiClient() {
  const apiKey = config.appdata.user.get('apiKey').startsWith('sk-')
    ? config.appdata.user.get('apiKey')
    : fs.existsSync(config.appdata.user.get('apiKey'))
    ? fs.readFileSync(config.appdata.user.get('apiKey'), 'utf8').trim()
    : ''
