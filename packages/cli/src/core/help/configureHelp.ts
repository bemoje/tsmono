...
import { getGlobalOptions } from '../CommandBuilder/getGlobalOptions'
import { prefixString } from '../util/prefixString'

export function configureHelp(cmd: CommandBuilder) {
  const cmdr = cmd.$
...
const opts = (() => {
  ...
  function commandUsage(this: Help, cmd: Command) {
    const args = cmd.registeredArguments.map((arg) => this.argumentTerm(arg)).join(' ')
    return [prefixString(cmd.builder) + ' ' + (args ? args : '[command]'), '[options]'].join(' ')
  }
