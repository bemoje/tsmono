...
export function createSearchPromptObject(name: string, data: string[], options: ISearchPromptOptions): PromptObject {
  const searchStopSequence = options.stopSearchSequence ?? ':'
  const keywordDelimiter = options.keywordDelimiter ?? ' '
  ...
  const filtering = options.filtering || { startsWith: false, includes: true }
...
async function search(
  ...
  if (filtering.includes) {
    ...
    temp.forEach((choice) => {
      ...
      if (!metaChoice) return
      const parsed = metaChoice.parsed
      if (everyKwInCmd(latestData.keywords, parsed)) {
        latestData.result.push(choice)
      }
    })
  }
 ...
 * Search 'startsWith'
 */
function someCmdStartsWithEveryKw(keywords: string[], parsed: string[]): boolean {
  let lastP = 0
 ...
 * Search 'includes'
 */
function everyKwInCmd(keywords: string[], parsed: string[]): boolean {
