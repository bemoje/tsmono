import { Any, JsonValue, promptUserEditJsonInTextEditorSync } from '@bemoje/util'
 ...
 */
export abstract class JsonFileSection<Val = JsonValue> {
  ...
  edit(editor?: string) {
    const original = this.getAll()
    const parsed = promptUserEditJsonInTextEditorSync(
      original as JsonValue,
      editor || getRootCommand(this.file.cmd).db.config.get<string>('editor')
    )
