...
export const program = new Command()
  ...
  .action((search = '') => {
    const dirnames = fs.readdirSync(config.userconfig.get('rootdir')).filter((dirname) => {
      return fs.statSync(path.join(config.userconfig.get('rootdir'), dirname)).isDirectory()
    })
    if (!search) {
      ...
      rl.question('#: ', (string: string) => {
        ...
        exec('code ' + path.join(config.userconfig.get('rootdir'), dirname))
        rl.close()
      })
      return
    }
    ...
    dirnames.forEach((dirname) => {
      if (found) return
      const fullpath = path.join(config.userconfig.get('rootdir'), dirname)
      if (!dirname.toLowerCase().includes(search.toLowerCase())) {
        return false
      }
      console.log('Opening repository: ', dirname)

      exec(`${config.userconfig.get('IDE')} ${fullpath}`)
      found = true
      return true
    })
    if (!found) {
